from data.mag import generate_author_updates
...
if __name__ == '__main__':
    ...
    response = s.collection('mag_papers').schema.fields.get_single('Author')
    if 200 != response.status_code:
        print('adding field for authors:')
        print(s.collection('mag_papers').schema.fields.add('Author', 'important_strings').json())

    print('sending documents')
    counter = 0
    batch_generator = batch_jsonl(generate_author_updates(),batch_size)
