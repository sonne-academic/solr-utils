from data.mag import generate_json_dict, CONF_SERIES_FILE, _ENCODING, read_gzip_lines, DATA_FOLDER
from data.mag.headers import ConferenceSeries
...
async def writer(output: asyncio.Queue):
    ...
    with gzip.open(DATA_FOLDER/ 'conference_series_updates.jsonl.gz', 'w') as file:
        while True:
...
def gen_conf_series():
    # counter = 0
    for x in generate_json_dict(ConferenceSeries,read_gzip_lines(CONF_SERIES_FILE,_ENCODING)):
        # counter +=1
        # if 10 == counter:
        #     break
        yield x
