from data.mag import generate_journal_updates, generate_conference_series_updates, generate_conference_instance_updates
...
if __name__ == '__main__':
    ...
    response = s.collection('mag_papers').schema.fields.get_single('ConferenceInstance')
    if 200 != response.status_code:
        print('adding field for journal:')
        print(s.collection('mag_papers').schema.fields.add('ConferenceInstance', 'important_string').json())

    print('sending documents')
    counter = 0
    batch_generator = batch_jsonl(generate_conference_instance_updates(),batch_size)
